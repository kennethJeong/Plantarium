//
//  bookPartsPitcherVC.swift
//  newNotitle1
//
//  Created by 정은후 on 2021/02/08.
//  Copyright © 2021 정은후. All rights reserved.
//

import UIKit

class bookPartsPitcherVC: UIViewController {
    ...
    // active imageViews
    var imageViewPitcher = UIImageView()
    var imageViewPitcherSide = UIImageView()
    
    var tagSuperTitle: String = "leaf"
    var tagMainTitle: String = "pitcher"
    ...
    override func viewDidLoad() {
        ...
        for (parts, partsImageView) in _dicActiveImageView {
            if parts.contains("side") {
                imageViewPitcherSide = partsImageView
                self.view.insertSubview(partsImageView, at: 0)
            } else {
                imageViewPitcher = partsImageView
        ...
        if self.imageViewPitcherSide.image != nil {
    ...
    @objc func clickCrossSection() {
        ...
        if !defaultCrossSection {
            ...
            UIView.animate(withDuration: 0.4, animations: {
                self.imageViewPitcher.frame.origin.x += self.imageViewPitcher.frame.width * 3.0
            })
